...
void UWeaponComponent::Fire(const FVector& BulletStart, const FVector& FireAtLocation)
{
	...
	// I wanted you to get used to the above pattern so you are thinking about where the code is running and handling
	// it differently depending on who is calling this function. However, in Unreal, you can simply call:
	// ServerFire(BulletStart, FireAtLocation);
	// If the server calls a Server function, it simply calls the function as normal and because it implements the same
	// behaviour as your if (GetOwnerRole)... block of code, you can just call ServerFire in all cases of Authority or
	// AutonomousProxy.

	// RPCs are a general concept outside of Unreal Engine and used regularly for networking tasks. Sometimes, this behaviour
	// of calling server functions from a server doesn't call anything, it just ignores it so it is worth understanding
	// the pattern I got you to code.
}
...
void UWeaponComponent::RepUpdateAmmoText()
{
	UpdateAmmoText();
}
...
void UWeaponComponent::UpdateAmmoText()
{
	if (APlayerCharacter* PlayerCharacter = Cast<APlayerCharacter>(GetOwner()))
	{
		...
		{
			PlayerCharacter->UpdateAmmoText(RoundsRemainingInMagazine, WeaponStats.MagazineSize);
		}
	}
}
